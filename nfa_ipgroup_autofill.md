...
- `school_id`
  - 优先：根据 `school_name` 到历史记录中查找最近（按 `update_time/create_time`）的非空 `school_id` 并沿用。
  - 若未找到：按全表当前最大纯数字 `school_id + 1` 自动生成并写入（不补零）。
  - 安全校验：若表内存在任意非纯数字 `school_id`（如 `A001` 或空字符串），脚本会报错退出，不执行更新。
...
- Q: 为什么脚本提示“检测到非数字 school_id”并退出？
  - A: 新院校 `school_id` 依赖“最大纯数字 + 1”分配。若表内存在非数字值，脚本会停止以避免错误分配。请先清理异常值后重试。
