...
@app.get("/api/jobs/{job_id}/log-tail", dependencies=[Depends(api_key_auth)])
async def job_log_tail(job_id: str, lines: int = Query(default=200, ge=1, le=2000)):
    with session_scope() as s:
    ...
    with p.open("r", encoding="utf-8", errors="replace") as f:
        content = "".join(deque(f, maxlen=int(lines)))
    return {"job_id": job_id, "lines": int(lines), "path": str(p), "content": content}


@app.get("/api/jobs/{job_id}/log", dependencies=[Depends(api_key_auth)])
async def download_job_log(job_id: str):
    ...
    if not p.exists():
        raise HTTPException(status_code=404, detail="Log not found")
    return FileResponse(path=str(p), filename=p.name, media_type="text/plain; charset=utf-8")
